...
\usepackage{textcomp} % Required for the upquote option
\usepackage{listings}
\lstset{
    basicstyle=\ttfamily,
...
\begin{frame}{The Complementary Filter}
    ...
    \begin{equation*}
        \hat{\theta}_{\text{fused}} = \theta_{\text{odom}} + \alpha_{\text{orientation}} \cdot \text{wrap}(\theta_{\text{imu}} - \theta_{\text{odom}})
    \end{equation*}
    ...
    \begin{equation*}
        \hat{\mathbf{p}}_{\text{fused}} = \mathbf{p}_{\text{odom}} + \alpha_{\text{position}} \cdot (\mathbf{p}_{\text{GPS}} - \mathbf{p}_{\text{odom}})
    \end{equation*}
\end{frame}
...
\begin{frame}{Filter Tuning Parameters}
    ...
    \begin{itemize}
        ...
        \item \textbf{$\alpha = 1$:} Pure absolute sensor (snaps to reading every tick, noisy and slow update rate).
...
\begin{frame}{Coordinate Frames \& GPS Alignment}
    \begin{itemize}
        \item \textbf{World/Arena Frame:} Origin $(0,0)$ at corner anchor~0; $x$-axis toward corner~1; $y$-axis toward corner~2 (toward the wall).
        \item \textbf{Starting orientation:} Robot must face the positive $y$ direction (towards the wall) $\Rightarrow$ \texttt{INITIAL\_THETA\_DEG = 90.0} in \texttt{main.py}.  No change needed if placed correctly at the taped start.
...
\begin{frame}[fragile]{Commands for Fusion Tuning}
    \item \textbf{Terminal 1 (inside Docker) --- start bridge every time:}
    \begin{verbatim}
ros2 run bridge bridge
    \end{verbatim}
    \item \textbf{Terminal 2 (inside Docker) --- run the desired test:}
    \begin{verbatim}
ros2 launch robot test_position_fusion.launch.py
# or
ros2 launch robot test_orientation_fusion.launch.py
    \end{verbatim}
    \item \textbf{Viewing results (outside Docker, in VS Code terminal):}
    \begin{itemize}
    ...
    \end{itemize}
    \begin{verbatim}
docker cp docker-ros2_runtime-1:
/root/orientation_fusion_test_result.png ~/Project-NUEVO/
    \end{verbatim}
\end{frame}

\begin{frame}{What to Tune in Each Test}
    \begin{itemize}
        \item \textbf{Position fusion:} edit \texttt{POSITION\_FUSION\_ALPHA} in \texttt{test\_position\_fusion.py}
        \item \textbf{Orientation fusion:} edit \texttt{ORIENTATION\_FUSION\_ALPHA} in \texttt{test\_orientation\_fusion.py}. 
        ...
        \item \textbf{Orientation test workflow:} robot turns 180 degrees; result saved inside Docker --- requires \texttt{docker cp} to view.
...
\begin{frame}{Key Parameters Reference}
    \begin{tabular}{lll}
        ...
        \texttt{INITIAL\_THETA\_DEG} & \texttt{main.py} & Leave at 90.0 (face $+y$, toward wall) \\
...
\begin{frame}{Visual Alpha-Tuning Checklist}
    ...
    \begin{itemize}
        ...
        \item Make sure to keep it between the coordinate markers so that GPS sees it. If GPS is never acquired, the run is not useful for alpha tuning.
    \end{itemize}
