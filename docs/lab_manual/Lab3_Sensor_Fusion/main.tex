...
\begin{itemize}
  ...
  \item \textbf{Rover markers (IDs 11--26):} Each robot group has one unique
...
\begin{notebox}
  ...
  These values are pre-measured for the current venue and are already set.
\end{notebox}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{%
    ...
    ros2\_ws/src/robot/robot/sensor\_fusion.py
      & \textbf{New} & OrientationComplementaryFilter, PositionComplementaryFilter \\[2pt]
...
source ros2_ws/docker/enter_ros2.sh rpi
...
\textbf{Procedure:}
\begin{enumerate}
  \item Navigate to your \texttt{Project\_NUEVO} directory and start the docker container
\begin{lstlisting} [style=bash]
COMPOSE=ros2_ws/docker/docker-compose.rpi.yml
docker compose -f $COMPOSE build
docker compose -f $COMPOSE up -d
\end{lstlisting}
  \item In one terminal, go into the docker container and
        start the bridge:
\begin{lstlisting}[style=bash]
source ros2_ws\docker\enter_ros2.sh rpi
ros2 run bridge bridge
...
\end{lstlisting}
  \item In a \textbf{second terminal} launch the test:
\begin{lstlisting}[style=bash]
source ros2_ws/docker/enter_ros2.sh rpi
ros2 launch robot test_orientation_fusion.launch.py
\end{lstlisting}
        The robot will make a $180^\circ$ turn.
...
\begin{warningbox}
  ...
  the current room.  Setting \texttt{ORIENTATION\_ALPHA = 0} means the robot
...
\end{lstlisting}sudo debugfs /dev/mmcblk0p2
  ...
  the ramp) --- see Section~\ref{sec:venue-coords}.sudo debugfs /dev/mmcblk0p2
...
\end{lstlisting}
  ...
  \item After the run, press \textbf{BTN\_2} to stop the robot processes and copy the trajectory plot to the host and view it in
